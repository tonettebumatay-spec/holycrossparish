    ...
    <div class="py-12 bg-gray-50 min-h-screen font-sans">
        <div class="max-w-7xl mx-auto px-10">
            ...
            <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6 rounded-r-lg">
                <p class="font-bold">🔍 Raw Data Debug:</p>
                <p>Count: {{ $appointments->count() }}</p>
                <pre class="bg-white p-2 rounded text-xs overflow-auto max-h-40">{{ json_encode($appointments->toArray(), JSON_PRETTY_PRINT) }}</pre>
            </div>
            ...
            <div class="bg-white rounded-[40px] shadow-sm border border-gray-100 p-12">
                @forelse($appointments as $index => $app)
                    <div class="border-b py-4">
                        <p><strong>#{{ $index + 1 }}</strong> – {{ $app->type ?? 'No type' }} – {{ $app->name ?? 'No name' }}</p>
                        <p>Date: {{ $app->appointment_date ?? 'No date' }}</p>
                        <p>Category: {{ $app->category ?? 'N/A' }}</p>
                        <p>Remarks: {{ $app->remarks ?? '' }}</p>
                        <p>Created: {{ $app->created_at ?? 'N/A' }}</p>
                    </div>
                @empty
